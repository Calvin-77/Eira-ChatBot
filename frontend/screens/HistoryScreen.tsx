...
const HistoryScreen = (): JSX.Element => {
  ...
  const handleDeleteChat = async (chatId: string) => {
    if (deleting[chatId]) return;
    setDeleting(prev => ({ ...prev, [chatId]: true }));
    try {
      const response = await axios.delete(`${SERVER_URL}/users/chats/${chatId}`);
      if (response.status === 200) {
        setChatHistory(prev => prev.filter(chat => chat._id !== chatId));
        Toast.show({
          type: 'success',
          text1: 'Chat Deleted',
          text2: 'Chat has been successfully deleted.',
          position: 'bottom',
          visibilityTime: 3000,
        });
      } else {
        Toast.show({
          ...
          text2: 'Could not delete the chat. Please try again later.',
          position: 'bottom',
          visibilityTime: 3000,
        });
      }
      ...
      Toast.show({
        type: 'error',
        text1: 'Deletion Failed',
        text2: 'Could not delete the conversation. Please check your connection and try again.',
        position: 'bottom',
        visibilityTime: 3000,
      });
    } finally {
